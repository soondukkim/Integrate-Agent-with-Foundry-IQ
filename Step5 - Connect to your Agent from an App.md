## Foundry Toolkit 에서 도구 호출에 대한 승인 설정
...
1. 현재 코드는 상담원과의 상호 작용을 관리하기 위해 대화 API를 사용합니다. 여기서 다음과 같은 기능이 구현됩니다 :
    ...
    - 승인/거부 후 `mcp_approval_response` 가 대화에 추가되고 새로운 답변이 생성됩니다.
    - 에이전트는 승인 결정을 바탕으로 Foundry IQ에서 정보를 검색합니다.
